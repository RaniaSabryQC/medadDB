...
public class LoginViaUaePassTest extends BaseTest {
    ...
    public void existingUsersWithAutomaticLinkingLevel3() throws IOException, InterruptedException {
        ...
        // Step 3: Create UAE Pass Identity Provider
        Allure.step("Step 3: Create UAE PASS Identity provider:{autoLinkingLevel3} Allow Automatic login for SOP1", () -> {
            System.out.println("\nStep 3: Creating UAE Pass identity provider...");
            JsonNode idpNode = getIdentityProviderManager()
        ...
        // Step 4: Create Test User
        Allure.step("Step 4: Create User :{testuser4}", () -> {
            System.out.println("\nStep 4: Creating test user...");
            JsonNode userNode = getUserManager().getUserNodeByUsername("users.json", "testuser4");
        ...
        assertTrue(getUserManager().userExists(testRealmName, "testuser4"));
        ...
        Allure.step("Step 6:  User login with match Level 3 | Email Address + Mobile Number ", () -> {
            ...
            captureScreenshot("Success linked user Login ", page);
            page.waitForURL(TEST_CLIENT_OIDC_CALLBACK_URL + "**");
            assertTrue(userManager.hasFederatedIdentity(testRealmName, userManager.getUserId(testRealmName,"testuser4"), "autoLinkingLevel3"));
    ...
    @Test
    @DisplayName("Successful login with Existing Users with Automatic Linking level 2")
    @Description("This test verifies that a user linked can successfully login directly through UAE Pass integration")
    @Severity(SeverityLevel.CRITICAL)
    @Story("UAE Pass implement SOP1, 2 and 3")
    public void existingUsersWithAutomaticLinkingLevel2() throws IOException, InterruptedException {
        // Step 1: Create Realm
        Allure.step("Step 1: Create Realm :{medad-allow}", () -> {
            System.out.println("\nStep 1: Creating realm...");
            JsonNode realmNode = getRealmConfigManager().getRealmNodeByName("realm-configs.json", "medad-allow");
        ...
        // Step 3: Create UAE Pass Identity Provider
        Allure.step("Step 3: Create UAE PASS Identity provider:{autoLinkingLevel} Allow Automatic login for SOP1", () -> {
            System.out.println("\nStep 3: Creating UAE Pass identity provider...");
            JsonNode idpNode = getIdentityProviderManager()
        ...
        Allure.step("Step 6:  User login with match Level 2 | Emirates ID or Unified ID", () -> {
            page.getByRole(AriaRole.LINK, new Page.GetByRoleOptions().setName(UAE_PASS_DISPLAY_NAME).setExact(true)).click();
            captureScreenshot("Success linked user Login ", page);
            page.waitForURL(TEST_CLIENT_OIDC_CALLBACK_URL + "**");
    ...
    @Severity(SeverityLevel.CRITICAL)
    @Story("UAE Pass implement SOP1, 2 and 3")
    public void successfulOverrideOfUserLinkAfterLogin() throws IOException, InterruptedException {
        // Step 1: Create Realm
        Allure.step("Step 1: Create Realm :{medad-allow}", () -> {
            System.out.println("\nStep 1: Creating realm...");
            JsonNode realmNode = getRealmConfigManager().getRealmNodeByName("realm-configs.json", "medad-allow");
        ...
        // Step 4: Create Test User
        Allure.step("Step 4: Create User :{testuser5}", () -> {
            System.out.println("\nStep 4: Creating test user...");
            userNode = getUserManager().getUserNodeByUsername("users.json", "testuser5");
            String userID = getUserManager().createUserFromNode(testRealmName, userNode);
            boolean hasFedLink = getUserManager().hasFederatedIdentity(testRealmName, userID, "uaepass");
            assertTrue(hasFedLink, "User should have federated identity link");
            System.out.println("✓ Federated identity link verified");
        ...
        assertTrue(getClientManager().clientExists(testRealmName, "test-client"));
        assertTrue(getIdentityProviderManager().identityProviderExists(testRealmName, "uaepass"));
        assertTrue(getUserManager().userExists(testRealmName, "testuser5"));
        openBrowser(MEDAD_IDENTITY_BASE_URL);
        ...
        Allure.step("Step 6:  User login with match Level 2 | Emirates ID or Unified ID", () -> {
            ...
            Assertions.assertNotEquals(userManager.getStringProperty(userNode,"federatedUserId")
                    , getUserManager().getFederatedUserIdByUsername(testRealmName,"testuser5","uaepass"));
        });
    ...
    @Description("This test verifies that a basic user can't login directly through UAE Pass integration")
    @Severity(SeverityLevel.CRITICAL)
    @Story("UAE Pass implement SOP1, 2 and 3")
    public void verifyBasicAccountLoginIsRestrictedSuccessfully() throws IOException, InterruptedException {
        // Step 1: Create Realm
        Allure.step("Step 1: Create Realm :{medad-allow}", () -> {
            System.out.println("\nStep 1: Creating realm...");
            JsonNode realmNode = getRealmConfigManager().getRealmNodeByName("realm-configs.json", "medad-allow");
        ...
        Allure.step("Step 4: Create User :{testuser3}", () -> {
            System.out.println("\nStep 4: Creating test user...");
            userNode = getUserManager().getUserNodeByUsername("users.json", "testuser3");
            String userID = getUserManager().createUserFromNode(testRealmName, userNode);
            System.out.println("✓ Federated identity link verified");
            Assertions.assertNotNull(userID, "User should be created");
        });
